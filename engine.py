...
def evaluate_hoi(dataset_file, model, postprocessors, data_loader, subject_category_id, device, args):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        # visualize
        # cross_attn_human = outputs['cross_attn_rel']  # [bs, nheads, num_queries, memory_len]
        # memory_len = cross_attn_human.shape[-1]
        # feat_h, feat_w = outputs.get('feat_h'), outputs.get('feat_w')
        # for b in range(cross_attn_human.shape[0]):
        #     img_tensor = samples.tensors[b].cpu()
        #     img_np = np.array(F.to_pil_image(img_tensor))
        #     img_id = targets[b]['id']
        #     img_filename = targets[b].get('filename', f'img_{img_id}.jpg')
        #     img_path = os.path.join(args.image_path, img_filename)
        #     # Compute attention map
        #     attn_map = cross_attn_human[b, 0].view(feat_h, feat_w).cpu().numpy()
        #     try:
        #         save_path = args.output_image
        #         os.makedirs(save_path, exist_ok=True)
        #         # Use visualize_grid_attention_v2 to generate heatmap
        #         visualize_grid_attention_v2(
        #             img_path=img_path,
        #             save_path=save_path,
        #             attention_mask=attn_map,
        #             ratio=1.0,  # No scaling, keep original size
        #             cmap="jet",
        #             save_image=True,
        #             quality=200
        #         )
        #     except Exception as e:
        #         print(f"Error generating heatmap for img_{img_id}: {e}")

        orig_target_sizes = torch.stack([t["orig_size"] for t in targets], dim=0)
        results = postprocessors['hoi'](outputs, orig_target_sizes)
        preds.extend(list(itertools.chain.from_iterable(utils.all_gather(results))))
        gts.extend(list(itertools.chain.from_iterable(utils.all_gather(targets))))
