...
class HICODetection(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        # 根据索引 idx，从 self.annotations 中获取对应图像的标注（通过 self.ids[idx]）
        img_anno = self.annotations[self.ids[idx]]
        # 使用 PIL 的 Image.open 打开 img_folder 中指定文件名（img_anno['file_name']）的图像
        img = Image.open(self.img_folder / img_anno['file_name']).convert('RGB')
        w, h = img.size

        # 在训练模式下，如果物体标注数量超过 num_queries，则截取前 num_queries 个标注
        if self.img_set == 'train' and len(img_anno['annotations']) > self.num_queries:
            img_anno['annotations'] = img_anno['annotations'][:self.num_queries]

        # 从每个物体标注中提取边界框（bbox），生成边界框列表
        ...
        if self.img_set == 'train':
            ...
            for hoi in img_anno['hoi_annotation']:
                ...
                if sub_obj_pair in sub_obj_pairs:
                    ...
                    # sub_center = torch.tensor([(origin_sub_box[0] + origin_sub_box[2]) / 2, (origin_sub_box[1] + origin_sub_box[3]) / 2])
                    # obj_center = torch.tensor([(origin_obj_box[0] + origin_obj_box[2]) / 2, (origin_obj_box[1] + origin_obj_box[3]) / 2])
                    # rel_vecs = torch.cat([sub_center, obj_center])
                    # # rel_box = box_xyxy_to_cxcywh(rel_box)  # (x,y,w,h)
                    # h_r, w_r = img.shape[-2:]
                    # rel_vecs = rel_vecs / torch.tensor([w_r, h_r, w_r, h_r], dtype=torch.float32)
